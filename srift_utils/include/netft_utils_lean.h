...
public:
  ...
  bool setUserInput(std::string world, std::string ft, double force, double torque);  //force 为ForceMaxU, torque为TorqueMaxU
  bool run();
  void stop();
  ...
  void setBias(geometry_msgs::Wrench data);
  bool isReady();
  ...
  void setFTTopic(std::string ftTopic);  //如果有topic的话,会从topic中读取,否则从sridriver Ethernet从直接读取
...
private:
  ...
  bool useYAMLfile;
  //netft class
  std::unique_ptr< SRI_ft_Driver::SRIDriver> netft;

  // cycle rate
  double cycleRate; //循环,hz
  ...
  geometry_msgs::WrenchStamped threshold;          // Wrench containing thresholds,当在这个小阈值范围内时设置为零
  ...
  void transformFrame(geometry_msgs::WrenchStamped in_data, geometry_msgs::WrenchStamped &out_data, char target_frame);//target_frame是'w'时,转到世界坐标系下的力矩数据,是't'时,从世界坐标系数据转到tool坐标系
  void checkMaxForce();
  bool monitorData();
